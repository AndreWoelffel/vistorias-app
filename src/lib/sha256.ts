/** SHA-256 em JS puro — funciona em HTTP (LAN) onde crypto.subtle não está disponível. */
...
function sha256Bytes(data: Uint8Array): Uint8Array {
  ...
  new DataView(withLen.buffer).setUint32(withLen.length - 4, ml, false);
  ...
  for (let off = 0; off < withLen.length; off += 64) {
    for (let i = 0; i < 16; i++) {
      W[i] = new DataView(withLen.buffer, off + i * 4, 4).getUint32(0, false);
    }
    for (let i = 16; i < 64; i++) {
